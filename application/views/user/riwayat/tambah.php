		...
		<div class="col-lg-12 mb-4">
			...
			<div class="card shadow mb-4">
				...
				<form action="<?= base_url('user/tambah_riwayat') ?>" id="form_tambah" method="POST" enctype="multipart/form-data">
						...
						<div class="form-group col-md-8 ml-3 mt-3">
							...
							<select class="form-control" name="jenis" id="jenis" onchange="autofill()">
								...
								<?php foreach ($mobil as $mbl) : ?>
									<option value="<?= $mbl['no_mobil']; ?>"><?= $mbl['jenis_mobil']; ?> - <?= $mbl['no_plat']; ?> - <?= $mbl['nama']; ?></option>
								<?php endforeach; ?>
							</select>
						</div>
					</div>
					...
					<input type="hidden" name="konfirmasi_sopir" value="Diproses">
					<input type="hidden" name="konfirmasi" value="Menunggu">

					<input type="hidden" name="tgl_upload" value="<?php
																	date_default_timezone_set('Asia/Jakarta'); // Zona Waktu indonesia
																	echo date('Y-m-d H:i:s'); //kombinasi jam dan tanggal
																	?>">
					<!-- <input type="hidden" name="no_sopir" value=""> -->
